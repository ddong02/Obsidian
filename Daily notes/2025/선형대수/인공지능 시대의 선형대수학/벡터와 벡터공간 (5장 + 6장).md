...
![[vector5.2-1.excalidraw|250]]

벡터 $\mathbf{v}$ 와 $\mathbf{w}$ 는 위의 그림과 같이 평행사변형을 연결하는 대각선으로 표현된다.
...
![[vector5.2-2.excalidraw|400]]

$\boldsymbol R^2$ 상에서의 $\mathbf{v+w}$ 의 좌표는 $(v_1 + w_1, v_2 + w_2)$ 이다.

- **벡터의 차**

$\boldsymbol R^2$ 상에서 $\mathbf{v} = (v_1, v_2, \cdots, v_n)$ 와  $\mathbf{w} = (w_1, w_2, \cdots, w_n)$ 가 주어졌을 때, **벡터의 차(difference)**또는뺄셈은 다음과 같다.
...
특히, $\alpha = 0$ 이거나 $\mathbf{u} = \mathbf{0}$ 이면 $\alpha \mathbf{u} = 0$ 이고, $(-1) \mathbf{u} = - \mathbf{u}$ 이다.

![[vector5.2-5.excalidraw|350]]
