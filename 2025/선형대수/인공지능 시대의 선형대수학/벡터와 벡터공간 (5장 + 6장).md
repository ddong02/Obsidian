...
다음의 10가지 공리들(axioms)을 만족할 때 $V$ 를 **벡터공간(vector space)**이라고 한다.

이때 공리들은 $V$ 안의 모든 벡터 $\mathbf{u,v,w}$ 와 모든 스칼라 $\alpha, \beta$ 에 대하여 성립해야 한다.

>1. $\mathbf{u}$ 와 $\mathbf{v}$ 의 합인 $\mathbf{u+v}$ 도 $V$ 에 속한다. (덧셈에 대해 닫혀있다)
>2. 모든 $\mathbf{u,v}$ 에 대하여 $\mathbf{u + v = v + u}$ (덧셈에 대한 교환법칙)
>3. $\mathbf{(u+v)+w=u+(v+w)}$ (덧셈에 대한 결합법칙)
>4. $\mathbf{u+0 = u}$ 인 영벡터 $V$ 가 존재한다. (영벡터의 존재)
>5. $V$ 상의 모든 $\mathbf{u}$ 에 대하여 $\mathbf{u + (-u) = 0}$ 을 만족하는 $\mathbf{-u}$ 가 존재한다. (덧셈에 대한 역원)
>6. $\mathbf{u}$ 에다 스칼라 $\alpha$ 를 곱한 $\alpha \mathbf{u}$ 도 $V$ 에 속한다. (스칼라 곱에 대해 닫혀있다)
>7. $\alpha \mathbf{(u+v)} = \alpha \mathbf{u} + \alpha \mathbf{v}$ (스칼라 곱에 대한 배분법칙)
>8. $(\alpha + \beta) \mathbf{u} = \alpha \mathbf{u} + \beta \mathbf{u}$ (스칼라 곱에 대한 배분법칙)
>9. $\alpha (\beta \mathbf{u}) = (\alpha \beta) \mathbf{u}$ (스칼라 곱에 대한 결합법칙)
>10. $1 \mathbf{u} = \mathbf{u}$ (1은 스칼라 곱의 항등원)
